...
class JSONEncoder(json.JSONEncoder):
    """
        JSON Encoder - handles special encoding and conversion
    """

    def default(self, o):
        if hasattr(o, "isoformat"):  # handles both date and datetime objects
            return o.isoformat()
...
class CacheManager:
    """
        CacheManager manages redis, postgres
        mssql caching
    """
    ...
    def setup_sql_db(self, server, db_name, username, password):
        ...
        self.sqldb = MSSQLInteraction(
            dbname=db_name, host=server, user=username, password=password
        )
        self.sqldb.conn(dict_cursor=True)
        self.sqldb.batch_open()
        return self
            ...
            if self.use_json:
                if hash_key:
                    ...
                    self.pipeline.hset(
                        name=temp_keyname,
                        key=hash_key,
                        value=json.dumps(current_key, cls=JSONEncoder),
                    )
                    if timestamp_key:
                        self.pipeline.hset(
                            name=temp_keyname,
                    ...
                    if version_key:
                        self.pipeline.hset(
                            name=temp_keyname, key=version_key, value=str(version)
                        )
                else:
                    self.pipeline.set(
                        name=temp_keyname, value=json.dumps(data, cls=JSONEncoder)
                    )
            else:
    ...
    def update_cache_from_sql(self, sql, prefix, key_field, TTL=0):
        ...
        for row in rows:
            ...
            if self.use_json:
                ...
                self.pipeline.set(
                    name=temp_keyname, value=json.dumps(current_key, cls=JSONEncoder)
                )
            else:
                self.pipeline.hmset(temp_keyname, data)
    ...
    @deprecated("We should call this from mssql_tools instead")
