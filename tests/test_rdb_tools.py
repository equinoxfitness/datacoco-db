...
class TestDBInteraction(unittest.TestCase):
    ...
    def test_url_from_conf(self):
        ...
        print(from_conf)
        self.assertEqual(True, from_conf == 'postgresql+psycopg2://test_user:test_password@test_host:test_port/test_db_name')

        conf = {'type': 'mssql',  'user': 'test_user', 'password': 'test_password', 'server': 'test_server', 'db_name': 'test_db_name'}
        from_conf = self.testClass.url_from_conf(conf)
        print(from_conf)
